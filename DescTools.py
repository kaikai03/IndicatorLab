...
########### benchmark samples #################
def hs300(start=None, end=None, gap=60):
    code_lsit = get_blocks_view('zs')['沪深300']
    data = get_data(codes_list, start=None, end=None, gap=60, freq=QA.FREQUENCE.DAY, market=MARKET_TYPE.STOCK_CN):
    return data
...
def add_industry(stocks_df, hy_source='swhy', inplace=True):
    '''向stock的DataFrame中插入行业数据
       ...
       :return: {pd.DataFame}
    '''
    industry = get_blockname_from_stock(stocks_df.index.levels[1], hy_source)
    if inplace:
        stocks_df['industry'] = stocks_df.index.get_level_values(1).map(industry)
        return stocks_df
    else:
        return stocks_df.assign(industry=a.index.get_level_values(1).map(industry))
...
#     )
    ...
    blocks_view = a[a['type'] == 'swhy'].groupby('blockname').apply(lambda x:x.index.to_list())
    return blocks_view
